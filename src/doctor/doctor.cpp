            ...
            if (ops[0] == QLatin1String("output")) {
                ...
                if (ops.count() == 3 && subcmd == QLatin1String("primary")) {
                    ...
                    if (brightness < 50 || brightness > 10000) {
                        qCWarning(KSCREEN_DOCTOR) << "Wrong input: Allowed range for sdr-brightness is 50 to 10000";
                        qApp->exit(9);
                        return;
                    }
                    ...
                    if (ops[3] == "preferEfficiency") {
                        output->setColorPowerPreference(Output::ColorPowerTradeoff::PreferEfficiency);
                    } else if (ops[3] == "preferAccuracy") {
                        output->setColorPowerPreference(Output::ColorPowerTradeoff::PreferAccuracy);
                    } else {
                        qCWarning(KSCREEN_DOCTOR) << "Wrong input: only allowed values for colorPowerTradeoff are \"preferEfficiency\" and \"preferAccuracy\"";
                        qApp->exit(9);
                        return;
                    }
                    m_changed = true;
                    ...
                    if (ops.count() > 6) {
                        if (ops[6] == "reduced") {
                            ...
                            qCWarning(KSCREEN_DOCTOR) << "Invalid input: Only 'reduced' or 'full' allowed";
                            qApp->exit(9);
                            return;
                        }
                    }
                    ...
                    m_changed = true;
                } else if (ops.count() == 4 && subcmd == "removeCustomMode") {
                    auto modes = output->customModes();
